...
│   ├── components/                  # 组件源码（w-* 前缀，含 layout/layout-header/layout-content/layout-footer/layout-sider 等布局组件，tree/tree-select 树形组件，list/list-item 列表组件，badge 徽标组件，stepper 步进器组件）
│   ├── libs/                        # 工具库（tempo 日期库等）
